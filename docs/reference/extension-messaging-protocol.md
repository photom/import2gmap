...
| `input[type="file"][accept*="KML"]`, falling back to any `input[type="file"]` in the frame (**2026-08-04**: fallback for picker v2's upload pane, whose own input markup was never directly observed) | **picker iframe** (`docs.google.com/picker...`) |
| `div[role="option"]` whose trimmed text is `アップロード`/`Upload` (**2026-08-04**: picker v2's upload-nav option, clicked only when the file input isn't already present) | **picker iframe** |
...
| Worker → Maps | `MAPS_FEED_KML` | `{ protocolVersion, jobId, kml: string, fileName: string }` — wait for either the file input or (picker v2, 2026-08-04) the upload-nav option; if only the nav option appeared, click it and wait again for the file input; then build the KML `File`, assign via `DataTransfer`, dispatch `change` | **picker** (discovered `frameId`) |
