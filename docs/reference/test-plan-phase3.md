...
- [ ] **15.2 KML import handling — split across two frames (2026-08-02 bugfix, superseding the single `MAPS_IMPORT_KML` design)**. Root cause: the KML upload dialog is a cross-origin `docs.google.com/picker` iframe, not more DOM in the top My Maps document — the file input is unreachable from the top frame (`document.querySelector('input[type="file"][accept*="KML"]')` from the top frame returns `null`; confirmed by a live user after the prepare/navigation bugfix above got the flow this far). See the [spike results addendum](../explanation/my-maps-import-spike-results.md) and [messaging protocol §6](extension-messaging-protocol.md#6-service-worker--my-maps-content-script). Replaced by three messages, each handled by whichever frame the relevant DOM actually lives in:
  ...
  - [ ] **`MAPS_FEED_KML` (picker frame only — `mymaps.content.ts` injected with `detectMapsFrameRole(location.hostname) === 'picker'`)**: builds the KML file (Module 14), assigns it via `DataTransfer`, dispatches `change`, replies `{ ok: true }` once a file input is found. Does not poll for the success signal itself (that selector is back in the top frame). **Updated 2026-08-04 — picker v2 source-nav layout (see Module 13.6, spike results addendum)**: the picker iframe has two observed layouts. Layout 1 (original spike): the file input (`input[type="file"][accept*="KML"]`, falling back to any `input[type="file"]` in the frame — the v2 upload pane's own input markup was never directly observed, so this fallback is defensive, not confirmed) is already present — used as-is with **no extra click** (clicking a nav when already on the upload pane could reset picker state). Layout 2 ("picker v2", `data-config` containing `"https://docs.google.com/picker/v2/"`): the picker instead renders a left source-nav listbox (`Google ドライブ` preselected), and the upload pane — and its file input — do not exist in the DOM until the nav's `アップロード`/`Upload` option (Module 13.6) is clicked. Since the picker frame may still be loading when `MAPS_FEED_KML` arrives, the handler polls (bounded by `PREPARE_TIMEOUT_MS`, no arbitrary short pre-check) until *either* the file input *or* the nav option appears; if only the nav option showed up, it clicks it and polls again (same bound) for the file input to render — the fourth recurrence of the rendering-lag rule (see spike results). Neither the file input nor the nav option ever appearing, or the file input never rendering after the nav click → `{ ok: false, code: 'MyMapsUiChanged' }`, same explicit-failure contract as before.
