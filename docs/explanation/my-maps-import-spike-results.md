...
| `input[type="file"][accept*="KML"]`, falling back to any `input[type="file"]` in the frame (the file input; fallback is defensive/unconfirmed for layout 2 — see above) | **picker iframe** (`docs.google.com/picker...`) |
| `div[role="option"]` whose trimmed text is `アップロード`/`Upload` (layout 2's upload-nav option, clicked only when the file input isn't already present) | **picker iframe** |
| `#ly0-layer-header .pbTTYe-r4nke` (success signal, below) | top |

Prefer the `accept` substring match over the `jsname` attribute for the file input, since `accept` is a meaningful, less likely to silently rename value; treat a missing match (in the picker frame, after also failing to find and use the layout-2 upload-nav option) as `MyMapsUiChanged`.

**Behavior across both layouts**: if the file input is already present (layout 1), use it directly — **no extra click**, since clicking a nav option when already on the upload pane could reset picker state. Otherwise (layout 2, or the frame still loading), poll until *either* the file input *or* the upload-nav option appears (bounded timeout, since the picker frame may not have finished loading yet when `MAPS_FEED_KML` arrives); if only the nav option showed up, click it and poll again (same bound) for the file input to render — this nav-click-then-render lag is the fourth recurrence of the rendering-lag rule (top-of-doc addendum). Neither ever appearing, on either poll, is `MyMapsUiChanged`.
...
- **The picker iframe has a second rendered layout ("picker v2") that hides the file input behind a source-nav click — resolved 2026-08-04, see the addendum at the top of this doc and the revised Section 3.** On some accounts the picker opens on a source-nav listbox (`Google ドライブ` preselected) instead of the upload pane, so the file input doesn't exist in the DOM at all until `アップロード` is clicked; `handleFeedKml`'s single `waitForElement(FILE_INPUT_SELECTOR, ...)` polled a DOM that would never contain it and timed out to `MyMapsUiChanged`. No new permission needed — same `docs.google.com/picker*` iframe, still targeted by the previously-discovered `frameId`. Fixed by making `MAPS_FEED_KML` bounded-poll for *either* the file input *or* the upload-nav option (`isPickerUploadNavLabel`, `src/domain/my-maps/my-maps-detectors.ts`), and if only the nav option appeared, clicking it and polling again for the file input — the fourth recurrence of the rendering-lag rule.
