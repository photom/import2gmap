...
**2026-08-05 third addendum — the second picker-v2 fix (previous addendum) still stalled in the field; addressed jointly with a coordinator-supplied "hypothesis 0".** A live user re-captured the stuck DOM (see `picker-v2-live-dom.md` snapshot referenced in this round's task) and reported the left source-nav still on 「Google ドライブ」, no upload pane/file input, and — critically — **no log output in the extension's console**, even though the map had demonstrably been created, renamed, and the import dialog opened. Two independent findings from that snapshot:

- The upload-nav option (`name="2"`, 「アップロード」) was captured at `tabindex="0"` — i.e. `option.focus()` had run, the only place this flow calls `.focus()` — while `aria-selected` stayed `"false"`. A bare `new KeyboardEvent('keydown', {key: 'Enter'})` leaves `keyCode`/`which` at `0` (they're not derived from `key`), and Closure/jsaction keydown handlers routinely branch on that legacy field, so the keyboard activation strategies were very likely no-ops.
- "No log output" was traced to a documentation bug, not a code bug: `mymaps.content.ts`'s own `logStep` comment claimed it "surfaces in the service-worker console", but a content script's `console.log` only ever reaches the console of the *frame it runs in* — for the picker frame, that's the invisible-to-the-user `docs.google.com` iframe, never the service worker. The flow's picker-frame trace (`activateUploadPaneNav`'s before/after lines, `handleFeedKml`'s layout/fileInput lines) was never visible anywhere the user was actually looking.

Fixes shipped this round (see `pickerActivationKeyEventInit`, `hasPickerFrameContent`, `isPickerFrameReady`, `isPickerSpinnerActive` in `src/domain/my-maps/my-maps-detectors.ts`; `activateUploadPaneNav`, `handleFeedKml`, `main()` in `entrypoints/mymaps.content.ts`; `waitForPickerFrame`, `sendFeedKmlWithRetry` in `entrypoints/background.ts`):

1. **Keyboard events now carry `keyCode`/`which`/`code`**, not just `key` — closes the gap the live DOM evidence pointed at directly.
2. **The pointer/mouse strategy is more realistic**: a hover sequence (`pointerover`/`mouseover`/`mousemove`) precedes the press/release, every event carries `view`/`detail`/`button`/`buttons` and the pointer-specific fields, and it targets the option's innermost child element rather than the `role="option"` div.
3. **Activation cycles through all four strategies repeatedly until an overall deadline**, not a one-shot pass — addresses the possibility that jsaction's event dispatcher for the listbox isn't installed yet the instant the nav option first appears (a late dispatcher silently drops early synthetic events); the "hypothesis 0" reasoning behind this: the picker card renders progressively, so several distinct timing failures upstream of activation would all look identical from the outside.
4. **A positive readiness gate**: the pre-activation poll now also requires the picker's own loading spinner (`jsname="aZ2wEe"`, finished markers `data-active="false"` / `aria-hidden="true"`) to not be active, not just "the nav option exists in the DOM".
5. **Picker-frame discovery (`waitForPickerFrame`) is now content-aware, not hostname-only**: `mymaps.content.ts`'s `main()` returns `{ role, hasPickerContent }` instead of a bare role string, and a frame only counts as the picker once it has actually rendered the nav option or the strict file input — closing a gap where the worker could latch onto a still-loading (or, defensively, a nested) `docs.google.com` frame and never revisit that choice.
6. **`MAPS_FEED_KML` delivery is now retried on "no reply at all"**: if the discovered picker frame's execution context is torn down before it replies (`sendToMapsTab` rejects or resolves `undefined` — as opposed to an explicit `{ok:false,code}` from a still-alive content script), the worker rediscovers the picker frame and resends, bounded by an overall deadline, rather than failing immediately. This is the leading explanation for "no log output at all" from a prior round: if the picker frame died before `handleFeedKml` ever ran, not one of its `logStep` lines could exist.
7. **Picker-frame diagnostics now reach the service-worker console**: `handleFeedKml`'s buffered `logStep` lines are attached to the `MAPS_FEED_KML_RESULT` reply's `diagnostics` field, and `entrypoints/background.ts` re-logs them on receipt — regardless of whether the picker frame's own console was ever visible to the user.

**Not independently re-verified against a live picker v2 account** (same caveat as the previous two addenda — no such account is available in this environment). `npm test` / `npm run compile` / `npm run build` all pass. If this still doesn't work in the field, the next diagnostic step is a fresh live DOM dump taken *immediately after* each activation attempt, plus checking whether the forwarded `diagnostics` lines now actually appear in the service-worker console (they should, regardless of what else fails) — their presence or absence narrows down whether `handleFeedKml` ran at all.
...
- **The third picker-v2 fix (2026-08-05 third addendum) has not been verified against a live picker v2 account either** — see that addendum for the fresh-DOM-dump-plus-diagnostics-check follow-up if it still doesn't work.
