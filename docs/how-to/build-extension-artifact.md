...
Output goes to `.output/chrome-mv3/` (gitignored). Example output from this repo:
...
`manifest.json`'s `name` / `description` / `version` are currently inherited from `package.json` (still the WXT starter placeholders — `wxt-react-starter` / `0.0.0`). Update `wxt.config.ts`'s `manifest` block (per [wxt-entrypoints-and-directory-structure](../reference/wxt-entrypoints-and-directory-structure.md)) before shipping a real build.
